...
/*
  Šių pratybų tikslas išspręsti užduotis panaudojant bendrinius tipus. [1-6]
...
const strings: string[] = ["sausis", "vasaris", "kovas", "balandis", "geguze", "birzelis", "liepa", "rugpjutis"];
...
console.group('5. Parašykite funkciją, kuri sujungia tokių pat tipų masyvus į vieną masyvą');
{
  ...
  const args2: Argument<string> = [['šiandien', 'yra', 'labai'], ['graži', 'rudens', 'diena']];
  const args3: Argument<boolean> = [[true, true, false], [false, true, false]];
...
console.group('6. Parašykite funkciją, kuri priimtų bet kokią reikšmę ir grąžintų objektą su savybėmis-funkcijomis "setValue" - reikšmei nustatyti ir "getValue" tai reikšmei nustatyti. Funkcijai perduota reikšmė neturi būti pasiekiama tiesiogiai.');
{
  ...
  const value1: number = 8;
  const value2: Array<string> = ["Kostas", "Bronius", "Mykolas"];
  const value3: { name: string, surname: string } = { name: 'Audra', surname: 'Valienie' };
  ...
  console.log({
    value1: obj1.setValue(1),
    value2: obj2.setValue(['pakeista']),
    value3: obj3.setValue({ name: 'Janina', surname: 'Mockute' }),
  });
}
console.groupEnd();
