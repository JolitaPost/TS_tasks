...
console.groupCollapsed('6. Suformuokite objektų masyvą su žmonių vardais ir pavardėm, kurie turi mašinas');
{
    ...
    const identities1 = peopleWithCars.map(createIdentity);
...
{
    const groupdrivingpeopleBySexReducer = (result, person) => {
        if (!person.hasCar)
            return result;
    ...
    const groupedPeopleBySex = people.reduce(groupdrivingpeopleBySexReducer, {});
    console.table(people);
...
console.groupCollapsed('9. Performuokite žmonių masyvą, jog kiekvieno žmogaus savybė "income", taptų "salary"');
{
    ...
    const lithuanianPeople = people.map(convertToEur);
    console.table(people);
