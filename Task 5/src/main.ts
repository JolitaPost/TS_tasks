type Person = {
  ...
  */
  ...
  console.groupCollapsed('6. Suformuokite objektų masyvą su žmonių vardais ir pavardėm, kurie turi mašinas');
  {
    ...
    const identities1: Identity[] = peopleWithCars.map(createIdentity);
    const identities2: Identity[] = people.reduce(identityReducer,[]);

    console.table(people);
    console.table(identities1);
  ...
  */
  {
    ...
    const groupdrivingpeopleBySexReducer = (result: drivingPeopleBySex, person: Person): drivingPeopleBySex => {
    ...
    const groupedPeopleBySex: drivingPeopleBySex = people.reduce(groupdrivingpeopleBySexReducer, {});

    console.table(people);
    console.table(groupedPeopleBySex);
  ...
  {
    type PersonLithuanian = Omit<Person, 'income'> & {
        salary?: Person['income']
      }
    
      const convertToEur = ({ income, ...person }: Person): PersonLithuanian => {
        const result: PersonLithuanian = { ...person };
      ...
      const lithuanianPeople: PersonLithuanian[] = people.map(convertToEur);
    
      console.table(people);
      console.table(lithuanianPeople);
